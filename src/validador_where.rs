    ...
    pub fn validar(&mut self) -> bool {
        ...
        for token in &self.tokens {
            match token.as_str() {
                ...
                ")" => {
                    self.parentesis_abiertos -= 1;
                    if self.parentesis_abiertos < 0 || matches!(ultimo_token, Some(ultimo) if self.operadores_binarios.contains(ultimo) || ultimo == "not" || ultimo == "(") {
                        return false;
                    }
                }
                "and" | "or" | ">" | "<" | "=" => {
                    if match ultimo_token {
                        None => true,
                        Some(ultimo) => self.operadores_binarios.contains(ultimo) || ultimo == "not" || ultimo == "(",
                    } {
                        return false;
                    }
                }
        ...
        self.parentesis_abiertos == 0 && matches!(ultimo_token, Some(ultimo) if !self.operadores_binarios.contains(ultimo) && ultimo != "not" && ultimo != "(")
    }
}
...
impl ValidadorOperandosValidos {
    ...
    pub fn validar(&self) -> Result<(),errores::Errores> {
        ...
        for operando in &self.operandos {
            if !self.campos_tabla.contains(&operando.to_lowercase()) && !self.es_literal(operando) && !operando.chars().all(char::is_numeric) {
...
mod tests {
    ...
    fn test_validador_expresiones_validas() {
        ...
        let tokens_validos1 = vec![
            "not".to_string(), "(".to_string(), "True".to_string(), "and".to_string(),
            "False".to_string(), ")".to_string(), "or".to_string(), "False".to_string()
        ];
        ...
        let tokens_validos2 = vec![
            "not".to_string(), "(".to_string(), "True".to_string(), "=".to_string(),
            "False".to_string(), "and".to_string(), "True".to_string(), ">".to_string(),
        ...
        let tokens_validos3 = vec![
            "(".to_string(), "True".to_string(), "and".to_string(), "False".to_string(),
            ")".to_string(), "or".to_string(), "(".to_string(), "not".to_string(),
            "True".to_string(), "or".to_string(), "False".to_string(), ")".to_string()
        ];
    ...
    fn test_validador_expresiones_invalidas() {
        ...
        let tokens_invalidos1 = vec![
            "not".to_string(), "(".to_string(), "True".to_string(), "and".to_string(),
            "False".to_string(), "or".to_string(), "False".to_string()
        ];
        ...
        let tokens_invalidos2 = vec![
            "True".to_string(), "and".to_string(), "or".to_string(), "False".to_string()
        ];
        ...
        let tokens_invalidos3 = vec![
            "True".to_string(), "and".to_string(), "False".to_string(), "or".to_string()
        ];
        ...
        let tokens_invalidos7 = vec![
            "(".to_string(), "not".to_string(), ")".to_string(), "True".to_string(),
            "and".to_string(), "False".to_string(), "or".to_string(), "False".to_string()
        ];
        ...
        let tokens_invalidos8 = vec![
            "True".to_string(), "and".to_string(), "not".to_string(), "(".to_string(),
            "or".to_string(), "False".to_string(), ")".to_string()
        ];
    ...
    fn test_pruebas_adicionales_invalidas() {
        ...
        for (i, tokens) in casos_invalidos.iter().enumerate() {
            ...
            assert!(!validador.validar(), "Error en la prueba {}: {:?} debería ser inválida", i, tokens);
        }
    }
    ...
    fn test_pruebas_adicionales_validas() {
        let casos_validos = vec![
            ...
            vec!["not", "(", "True", "=", "False", "and", "True", ">", "False", ")"],
            vec!["(", "True", "and", "False", ")", "or", "(", "not", "True", "or", "False", ")"],
            vec!["not", "(", "True", "=", "False", ")", "and", "True", "or", "False"],
            vec!["(", "(", "True", "or", "False", ")", "and", "not", "(", "False", "or", "True", ")", ")"],
        ];

        for (i, tokens) in casos_validos.iter().enumerate() {
            let tokens : Vec<String> = tokens.iter().map(|&t| t.to_string()).collect();
            let mut validador = ValidadorSintaxis::new(&tokens);
            assert!(validador.validar(), "Error en la prueba {}: {:?} debería ser válida", i, tokens);
        }
    }
}